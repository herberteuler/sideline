...
PATCHES=(
	...
	"${FILESDIR}"/0003-Disambiguate-ref-qualified-overloads-in-pybind11-bin.patch
)
...
src_compile() {
	cmake_src_compile
	if use python; then
		local spdlog_dir=( "${BUILD_DIR}"/libmamba-spdlog/CMakeFiles/Export/* )
		cp "${BUILD_DIR}/libmamba-spdlog/libmamba-spdlogConfig.cmake" "${spdlog_dir[0]}"
		mkdir -p "${BUILD_DIR}/libmamba-spdlog/include"
		cp -r "${S}/libmamba-spdlog/include/"* "${BUILD_DIR}/libmamba-spdlog/include"
		cd libmambapy
		export libmamba_DIR="${BUILD_DIR}/libmamba"
		export CMAKE_ARGS="-Dlibmamba-spdlog_DIR=${spdlog_dir[0]}"
